...
void CRAWDevelopDlg::OnBnClickedOpenFile()
{
	if( m_pDrawImage == NULL )
		return;
	static char BASED_CODE gbszFilter[] = "CR3 Files (*.CR3)|*.CR3|CR2 Files (*.CR2)|*.CR2|CRW Files (*.CRW)|*.CRW|TIF Files (*.TIF)|*.TIF|AllFiles (*.*)|*.*||";
	CFileDialog dlg(TRUE,"*.CR3",NULL,OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,gbszFilter);
	if( dlg.DoModal() == IDOK )
	{
		m_width=0;
